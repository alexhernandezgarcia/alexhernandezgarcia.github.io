...
title: IFT 6760B A26 - Introduction to GFlowNets I
---

name: gflownets-intro-part1-20260910
class: title, middle

## GFlowNets: Sampling as sequential decision making
### IFT 6760B A26

#### .gray224[September 10th - Session 3]
...
$$
p_{\theta}(x) = \frac{e^{-E(x)}}{Z} = \frac{e^{s(x)}}{Z} = \frac{e^{\beta s(x)}}{Z}
$$
...
$$
p_{\theta}(x) = \frac{e^{-E(x)}}{Z} = \frac{e^{s(x)}}{Z} = \frac{e^{\beta s(x)}}{Z} = \frac{R(x)}{Z}
$$
...
.conclusion-float[Our problem setting is very close to that of energy-based generative models.]
...
.conclusion-float[Our problem setting is very close to that of MCMC too.]
...
.center[![:scale 60%](../../../assets/images/teaching/gflownets/rl/rl_schematic.png)]

.conclusion-float[The typical problem setting of RL is different to ours, but there is also a notion of generating trajectories (objects $x$) with good rewards.]
