...
title: IFT 6760B A26 - Review of relevant background 
---

name: review-20260903
class: title, middle, hide-slide-number

## GFlowNets: Sampling as sequential decision making
### IFT 6760B A26

#### .gray224[September 3rd - Session 2]
...
- You will have a fresh overview of topics that are related to the rest of the course.
- You will have tools to situate within the literature the upcoming content.
...
- generating data
- estimating a density (outlier detection, data compression, etc.) as in KDE
- imputation (missing data)
- structure discovery (discovering latent variables)
- ...
...
--
- .h1[Latents]: Does the model use a latent vector $z$ to generate $x$? If so, is the representation of $z$ compressed with respect to $x$?
...
.conclusion-float[The main taxonomical factors have not changed dramatically from 2017 to 2023.]
...
- Advantages: The energy function is not restricted to specific architectures, as it can be any model that returns a scalar.
...
- Ask about challenges or disadvantages of this formulation? (partition function)
...
is easy to convert structured data into noise.

- Diffusion / forward process: data to noise (diffuse the data). This is easy.
- Reverse process: noise to data. This is learnt.
...
.references[
Credit to [Esmeralda Whitammer](https://whitammer.github.io/)'s wonderful slides.
]
...
- GANs do not specify an explicit log-likelihood function $\log p_{\theta}(x)$.
...
Importance sampling is a sampling method to approximate expectations:
